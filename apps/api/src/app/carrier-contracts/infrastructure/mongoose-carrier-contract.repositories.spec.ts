...
import {
  MongooseCarrierContractProjectDirectory,
  MongooseCarrierContractRepository,
} from './mongoose-carrier-contract.repositories';
...
describe('MongooseCarrierContractRepository', () => {
  it('maps a scoped facet result and empty result without N+1 calls', async () => {
    ...
    await expect(
      repo.list({ actorId, canManageAll: true, page: 1, limit: 20, skip: 0 }),
  ...
  it('returns null for invalid ids and maps optional contract terms', async () => {
    ...
    const repo = new MongooseCarrierContractRepository({ aggregate } as never);
    await expect(
      repo.findByIdWithAccess('invalid', actorId, true),
    ).resolves.toBeNull();
    const found = await repo.findByIdWithAccess(
      contractId.toString(),
      actorId,
      false,
    );
  ...
  it('creates and updates records then reloads the stable response', async () => {
    const model = {
      create: jest.fn().mockResolvedValue({ id: contractId.toString() }),
    ...
    await repo.update(
      ...
      { unitPrice: null, endDate: new Date('2026-12-31'), updatedBy: actorId },
    );
    expect(model.findByIdAndUpdate).toHaveBeenCalledWith(
      contractId.toString(),
      expect.objectContaining({ $unset: { unitPrice: 1 } }),
      { runValidators: true },
    );
  });
});
...
describe('MongooseCarrierContractProjectDirectory', () => {
  it('validates ids and maps globally managed and member-scoped projects', async () => {
    const aggregate = jest
      .fn()
      .mockResolvedValue([
        { _id: projectId, code: 'IDS-01', name: 'Riverside', unitCount: 200 },
      ]);
    const directory = new MongooseCarrierContractProjectDirectory({
      aggregate,
    } as never);
    await expect(
      directory.findByIdWithAccess('bad', actorId, true),
    ).resolves.toBeNull();
    await expect(
      directory.findByIdWithAccess(projectId.toString(), actorId, false),
    ).resolves.toEqual({
    ...
    aggregate.mockResolvedValueOnce([]);
    await expect(
      directory.findByIdWithAccess(projectId.toString(), actorId, true),
    ).resolves.toBeNull();
  });
});
